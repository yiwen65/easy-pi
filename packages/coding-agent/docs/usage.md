...
Background bash tasks are built in (this fork): `bash(run_in_background=true)` starts a command in the background and returns a task ID; in interactive mode a foreground command exceeding 60s (explicit timeout, max 300s) is promoted to a background task instead of being killed. Tasks are managed with the `task_list`, `task_output`, `task_stop`, and `wait_for` tools; completion is reported at the next request boundary; the status bar and the read-only `/tasks` panel show active tasks. Background tasks have no runtime cap by default — long tasks keep running until they finish or are stopped; set `backgroundBashTaskTimeoutSeconds` to opt into a hard cap (`0`, the default, means no cap, and an explicit `timeout` on the bash call caps that task only). A task with no output for `backgroundBashStallTimeoutSeconds` (default 1800 = 30 minutes, `0` disables) triggers a stall notice to the model and the UI but keeps running; `task_list` rows show `silent=<duration>` and the UI marks stalled tasks with `⏸`. Resource bounds: `backgroundBashMaxTasks` (default 8 concurrent tasks, `0` = no cap) and `backgroundBashMaxLogBytes` (default 64MB per task log, `0` = unlimited; a capped log ends with a truncation marker). `backgroundBashCompletionDelivery` chooses how a finished task reaches the model — `nextRequest` (default), `followUp` (queue behind the current turn), or `wake` (also start a turn when the session is idle). Every completion notice carries the result itself (status, exit code or signal, duration, command, log path); output is optional evidence: `backgroundBashCompletionInlineOutput` picks how much of it is inlined — `failures` (default: last `backgroundBashCompletionInlineBytes`, default 4096, of a failed/timed-out or log-broken task), `always` (successes too), `tail-lines` (last few lines only), or `never` (result + pointer only). Note that `succeeded`/`failed` reflect the shell exit code, not whether the command achieved its goal: `npm test || true` succeeds, and a non-zero exit from `grep`/`diff` can be normal. Events and extensions can observe task lifecycle through `background_task_started` / `background_task_completed` / `background_task_stalled`.
