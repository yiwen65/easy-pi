...
- **Footer** - Working directory, session name, total token/cache usage (`↑` input, `↓` output, `R` cache read, `W` cache write, `CR` latest cached-input token ratio), cost, context usage, current model. Totals include assistant responses, usage reported by tools, and summary generation.
...
- **Update check:** Easy Pi does not use the official Pi update feed. Automatic and self-update checks are disabled until `EASY_PI_UPDATE_URL` is set to an Easy Pi latest-version API endpoint. The endpoint returns JSON with `version` and optional `packageName`, `note`, and `changelogUrl` fields. Disable a configured automatic check with `PI_SKIP_VERSION_CHECK=1`.
- **Install/update telemetry:** after first install or a changelog-detected update, sends an anonymous version ping to `https://pi.dev/api/report-install`. This setting also controls optional provider attribution headers for OpenRouter, Cloudflare, and direct NVIDIA NIM requests. Opt out by setting `enableInstallTelemetry` to `false` in `settings.json`, or by setting `PI_TELEMETRY=0`. This does not control Easy Pi update checks.
...
**Background bash tasks (this fork).** Long-running commands can run in the background: `bash(run_in_background=true)` returns a task ID immediately, and in interactive mode a foreground command that exceeds 60s (explicit timeout, max 300s) is not killed — it keeps running as a background task. Manage tasks with `task_list`, `task_output` (≤32KB tail preview plus the full log path), `task_stop` (SIGTERM, then SIGKILL after 5s), and `wait_for` (block up to 600s). Task completion is reported as a synthetic message at the next request boundary, the status bar shows the active task count, and `/tasks` opens a read-only panel. Background runtime is uncapped by default, so long tasks keep running until they finish or are stopped; set `backgroundBashTaskTimeoutSeconds` to opt into a hard cap, and a task that produces no output for `backgroundBashStallTimeoutSeconds` (default 1800 = 30 minutes, `0` disables) only raises a stall notice — it is never stopped automatically. Concurrency and per-task log growth are bounded by `backgroundBashMaxTasks` (default 8) and `backgroundBashMaxLogBytes` (default 64MB), and `backgroundBashCompletionDelivery` picks how completions reach the model (`nextRequest` default, `followUp`, or `wake`). Extensions can subscribe to `background_task_started` / `background_task_completed` / `background_task_stalled`. For full isolation and persistence beyond the session, use tmux or containers.
...
| `--mode json --json-profile compact` | Output bounded observer events without tool payloads or aggregate history |
...
| `PI_SKIP_VERSION_CHECK` | Skip the Easy Pi version update check at startup |
| `EASY_PI_UPDATE_URL` | Easy Pi latest-version API URL. Automatic update checks are disabled when unset |
